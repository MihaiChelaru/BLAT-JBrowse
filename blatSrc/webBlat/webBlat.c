...
Extract sequence name as "ret" to pass to JBrowse
*/
char str[50];
strcpy(str, psl->tName);
char *ret;
ret = strchr(str, ':');
ret++;
/*
Change this path in order to specify where JBrowse is installed on your machine/server
Recommend placing the main jbrowse directory within whatever the root HTML folder is
Default for apache2 using Windows 10 Ubuntu is "/var/www/html/"
*/
char *pathToJBrowse;
pathToJBrowse = "/jbrowse/jbrowse-1.13.1-release/?data=sample_data/json/ecoli";
/* 
Create JBrowse hyperlink and new track labelled "Hits"
This will automatically display the "Hits" track when JBrowse is launched from the hyperlink
*/
fprintf(f, "<A HREF=\"..%s&tracks=Hits&loc=%s%%3A%d..%d&"
    "addFeatures=%%5B%%7B%%22seq_id%%22%%3A%%22%s%%22%%2C%%22start%%22%%3A%d%%2C%%22end%%22%%3A%d%%2C%%22name%%22%%3A%%22Hits%%22%%2C%%22type%%22%%3A%%22match%%22%%2C%%22"
    "subfeatures%%22%%3A%%5B%%7B%%22seq_id%%22%%3A%%22%s%%22%%2C%%22start%%22%%3A%d%%2C%%22end%%22%%3A%d%%2C%%22type%%22%%3A%%22match_part%%22%%7D%%5D%%7D%%5D&"
    "addTracks=%%5B%%7B%%22label%%22%%3A%%22Hits%%22%%2C%%22type%%22%%3A%%22JBrowse%%2FView%%2FTrack%%2FCanvasFeatures%%22%%2C%%22store%%22%%3A%%22url%%22%%2C%%22"
    "subParts%%22%%3A%%22match_part%%22%%2C%%22glyph%%22%%3A%%22JBrowse%%2FView%%2FFeatureGlyph%%2FSegments%%22%%7D%%5D&highlight=\">JBrowse</A><BR>\n", 
    pathToJBrowse, ret, psl->tStart-200,psl->tEnd+200, ret, psl->tStart, psl->tEnd, ret, psl->tStart, psl->tEnd);
carefulClose(&f);
